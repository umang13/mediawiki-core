...
class WebVideoTranscodeJob extends Job {
	 ...
	 *
	 */
	// Just take the first and last 750 bytes of the $error string and save them into $short_error string. Use this string now to update the transcode status db.
	private function setTranscodeError( $transcodeKey, $error ){
		...
		if(strlen($error)>1500)
		{
			for($i=0;$i<750;$i++)
			{
				$short_error[$i] = $error[$i];
			} 
			for($i=1500, $j=strlen($error)-750; $i>=750, $j<=strlen($error); $i--, $j++)
			{
				$short_error[$i] = $error[$j];
			}
		}
		else
		{
